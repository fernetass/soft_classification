...
def euclidean(a,b):
    #calculo de la distacia euclidea entre 2 puntos.     
    result = (a-b)**2
...
def weights_matrix(img,win_size):
    #calculo de la Matriz de ponderacion
    (nrows, ncols, nbands) = img.shape
    M = nrows*ncols
    w =zeros((M))
...
################################################################################
# Weighted Fuzzy C-Means class
################################################################################
class WFuzzyCMeans(FuzzyCMeans):
    '''
    Clase derivada de clase base FuzzyCmeans, es usada para instanciar el metodo WFCM.
    '''
