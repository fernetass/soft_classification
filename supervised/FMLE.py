...
class FuzzyGaussianClassifier(Classifier):
    # '''Clasificador  de fuzzy Maximum Likelihood '''
    ...
    def classify_spectrum(self, x):
        ...
        for cl in self.classes:
            ...
            # if first or prob[0,0] > max_prob:
                # first = False
                # max_prob = prob[0,0]    
                # max_class = cl.ind_clas[cl.index]
                
                
        
  # a=1.0/(((2.0*!PI)^(bands/2.0))*(sqrt(abs(determ(covmatrix)))))
  # c=transpose(z-mvector)
  # d=invert(covmatrix)
  # e=(z-mvector)
  # value=a*exp(-0.5*c#d#e)
        ...
        ind=argmax(degree)
        # print ind
        max_class= argmax(degree)#cl.ind_clas[indexs[ind]]
        #print max_class
